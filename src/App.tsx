...
// Lazy-loaded: only a signed-in visitor who actually opens one of these
// operator-only routes downloads this code (fixes PERF-01).
const DashboardPage = lazy(() => import("./AdminPages").then((m) => ({ default: m.DashboardPage })));
const ClientsPage = lazy(() => import("./AdminPages").then((m) => ({ default: m.ClientsPage })));
const AnalyticsPage = lazy(() => import("./AdminPages").then((m) => ({ default: m.AnalyticsPage })));
const StableSitePage = lazy(() => import("./AdminPages").then((m) => ({ default: m.StableSitePage })));
const IntegrationsPage = lazy(() => import("./AdminPages").then((m) => ({ default: m.IntegrationsPage })));
...
export function routeFromHash(hash: string = currentHash()): Route {
...
function RequireAuth({ children }: { children: React.ReactNode }) {
  ...
  return <Suspense fallback={<main className="auth-loading"><p>جارٍ التحميل...</p></main>}>{children}</Suspense>;
}
